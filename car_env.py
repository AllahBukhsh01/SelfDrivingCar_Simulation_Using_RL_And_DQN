...
#         # Car setup
#         self.car_pos = np.array([400.0, 300.0], dtype=np.float64)
#         self.car_angle = 0
#         self.car_speed = 0
#         self.max_speed = 8
...
#         # Checkpoints
#         self.goal = np.array([700, 300])

#     def reset(self):
#         self.car_pos = np.array([100, 300])
#         self.car_angle = 0
#         self.car_speed = 0
#         obs = np.array([*self.car_pos, self.car_speed, self.car_angle], dtype=np.float32)
#         return obs
...
#         # Distance to goal
#         distance = np.linalg.norm(self.goal - self.car_pos)
...
#         if (
#             self.car_pos[0] < 0
#             or self.car_pos[0] > self.width
#             or self.car_pos[1] < 0
#             or self.car_pos[1] > self.height
#         ):
#             reward -= 50
#             done = True
...
#         pygame.draw.circle(self.screen, (0, 255, 0), self.goal.astype(int), 10)
#         car_rect = pygame.Rect(self.car_pos[0] - 10, self.car_pos[1] - 10, 20, 20)
#         pygame.draw.rect(self.screen, (255, 100, 100), car_rect)
#         pygame.display.flip()
#         self.clock.tick(30)
...
# class CarEnv:
#     """
#     Lightweight 2D Track environment (PyGame).
#     - Agent controls: 0=left, 1=straight, 2=right, 3=accelerate, 4=brake
#     - Observation: [x, y, vx, vy, speed, angle, dist_to_goal, closest_obs_dx, closest_obs_dy]
#     """

#     def __init__(self, render_mode=True, track_width=300):
#         self.render_mode = render_mode
#         pygame.init()
#         if self.render_mode:
...
#         # Track geometry: a simple loop (rectangle with rounded corners)
#         self.track_margin = 100
#         self.track_width = track_width
#         self.finish_x = SCREEN_W - 120
#         self.finish_line = (self.finish_x, SCREEN_H // 2)
...
#         # Visual
#         self.car_image = None  # you can load sprite here
#         # If no sprite, we'll draw rectangle

#         # Action / observation specs
#         self.action_space = 5  # discrete
#         # We'll not implement gym.spaces to keep simple
...
#             self.opponents.append({"pos": np.array([x, y], dtype=np.float64), "speed": speed, "dir": -1})

#     def reset(self):
#         # Place agent near left of screen, center lane
#         self.car_pos = np.array([self.track_margin + 50.0, SCREEN_H // 2], dtype=np.float64)
#         self.car_angle = 0.0  # degrees, 0 = right
#         self.car_speed = 0.0
#         self.max_speed = 6.0
#         self.accel = 0.25
#         self.brake = 0.4
...
#         obs = np.array([
#             float(self.car_pos[0]),
#             float(self.car_pos[1]),
...
#         # straight does nothing to angle
...
#         # update opponents - simple lane movement (move left across screen)
#         for o in self.opponents:
#             o["pos"][0] += o["dir"] * o["speed"]
#             # wrap around
#             if o["pos"][0] < self.track_margin + 20:
#                 o["pos"][0] = SCREEN_W - self.track_margin - 20
#             if o["pos"][0] > SCREEN_W - self.track_margin - 20:
#                 o["pos"][0] = self.track_margin + 20

#         # compute reward
#         reward = 0.0
#         # progress reward: moving towards finish (x increasing)
#         prev_dist = getattr(self, "_prev_dist", None)
#         dist_to_goal = self.finish_x - self.car_pos[0]
#         if prev_dist is None:
#             reward += 0.0
#         else:
#             reward += (prev_dist - dist_to_goal) * 0.1  # small reward for progress
#         self._prev_dist = dist_to_goal

#         # off-road penalty: if y goes too far off center
#         road_top = SCREEN_H//2 - self.track_width//2
#         road_bottom = SCREEN_H//2 + self.track_width//2
#         if self.car_pos[1] < road_top or self.car_pos[1] > road_bottom:
#             reward -= 0.5  # penalty for being off road
#             offroad = True
#         else:
#             offroad = False
...
#         # finish condition
#         if self.car_pos[0] >= self.finish_x:
#             reward += 200.0
#             self.done = True

#         # small time penalty to encourage faster finish
#         reward -= 0.05
...
#         obs = self._get_obs()
#         return obs, float(reward), bool(self.done), {"collision": collision, "offroad": offroad}

#     def _get_car_rect(self):
#         # approximate rectangle for collision / drawing usage
#         w, h = 28, 18
#         return pygame.Rect(int(self.car_pos[0] - w/2), int(self.car_pos[1] - h/2), w, h)

#     def render(self):
#         # handle events so window is responsive
#         for event in pygame.event.get():
#             if event.type == pygame.QUIT:
#                 pygame.quit()
#                 raise SystemExit()

#         # draw background (asphalt)
#         self.screen.fill((60, 60, 60))
#         # draw track area
#         pygame.draw.rect(self.screen, (40, 40, 40),
#                          (self.track_margin, SCREEN_H//2 - self.track_width//2,
#                           SCREEN_W - 2*self.track_margin, self.track_width))
#         # finish line
#         pygame.draw.line(self.screen, (255, 255, 255),
#                          (self.finish_x, SCREEN_H//2 - self.track_width//2 - 10),
#                          (self.finish_x, SCREEN_H//2 + self.track_width//2 + 10), 4)
...
#         # draw agent car (rotated)
#         car_rect = self._get_car_rect()
#         # if sprite provided, rotate and blit; else draw rectangle rotated manually
...
#                 o["pos"][0] = left_bound
#             # ensure opponents stay inside road vertically (snap to lane)
...
#             pass
#         else:
#             # reward for reducing distance to finish (progress)
#             # reward += (self._prev_dist - dist_to_goal) * 0.15
#             progress = (self._prev_dist - dist_to_goal)
#             reward += progress * 0.6  # Stronger reward for forward movement
#         self._prev_dist = dist_to_goal

#         # checkpoint reward: if agent crosses a checkpoint x value and it's first time
#         for idx, cp_x in enumerate(self.checkpoints):
#             # checkpoint region: crossing from left to right near cp_x
#             if (self.car_pos[0] >= cp_x) and (idx not in self.passed_checkpoints):
#                 self.passed_checkpoints.add(idx)
#                 reward += 20.0  # checkpoint bonus
#                 reward += 0.5 * len(self.passed_checkpoints)  # continuous bonus
#                 print(f"[CarEnv] Passed checkpoint {idx+1} at x={cp_x}")
                
#         # -----------------------
#         # Track membership (strict)
#         # -----------------------
#         road_top = SCREEN_H//2 - self.track_width//2
#         road_bottom = SCREEN_H//2 + self.track_width//2
#         offroad = False
#         if not self._is_on_track():
#             # If car leaves the drivable band, treat as off-road and terminate.
#             offroad = True
#             reward -= 20.0          # strong penalty for leaving the track
#             self.done = True
#             print("[CarEnv] Off-road! Episode terminated to prevent cheating.")
...
#             reward -= 30.0
#             self.done = True
#             print("[CarEnv] Collision detected!")

#         # -----------------------
#         # finish condition (strict)
#         # only allow finish if car is on track and has passed all checkpoints
#         # -----------------------
#         if (self.car_pos[0] >= self.finish_x) and (len(self.passed_checkpoints) == len(self.checkpoints)) and (not offroad):
#             reward += 250.0
#             self.done = True
#             print("[CarEnv] Finish reached!")

#         # -----------------------
#         # small additional shaping
#         # -----------------------
...
#         if self.car_speed < 0.2:
#             reward -= 0.3  # discourage stalling
